...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('/produtos.json')
        ...
        .then(data => {
            ...
            data.forEach(item => {
                const row = document.createElement("tr");
                row.innerHTML = `<td>${item.produto}</td><td>${item.quantidade}</td>`;
                tableBody.appendChild(row);
            });
        })
    ...
    // Outras funções (exemplo de mostrar e ocultar modal)
    window.showAddItemForm = function() {
        document.getElementById("modal").style.display = "block";
    };

    window.cancelAddItem = function() {
        document.getElementById("modal").style.display = "none";
    };

    window.submitNewItem = function() {
        // Implementação para adicionar novo item
        cancelAddItem();
    };
});
...
document.addEventListener("DOMContentLoaded", function () {
    // Função para carregar os itens do JSON
    fetch('/static/json/produtos.json')
        .then(response => {
            if (!response.ok) {
